...
<head>
<meta charset="utf-8">
<title>Boletas de Cobro — {{ $hoy->format('d/m/Y') }}</title>
<style>
...
/* Cada cliente ocupa una página A4 */
.pagina {
    width: 210mm;
    height: 297mm;
    margin: 0 auto;
...
table.facturas tr.vencida td { color: #c00; }
...
/* Pantalla: portada antes de imprimir */
.no-print {
    font-family: Arial, sans-serif;
    padding: 20px;
    background: #f9fafb;
...
{{-- Cabecera visible solo en pantalla --}}
<div class="no-print" style="display:flex;align-items:center;justify-content:space-between;">
    <div>
        <strong style="font-size:18px;color:#1a56db;">Boletas de Cobro</strong>
        <span style="color:#555;margin-left:10px;">{{ $hoy->locale('es')->isoFormat('D [de] MMMM [de] YYYY') }}</span>
        <span style="background:#1a56db;color:white;border-radius:999px;padding:2px 10px;font-size:13px;margin-left:10px;">
            {{ $datos->count() }} cliente(s) con deuda pendiente
        </span>
    </div>
    <button onclick="window.print()"
            style="background:#1a56db;color:white;border:none;padding:10px 24px;border-radius:8px;font-size:14px;font-weight:bold;cursor:pointer;">
        🖨 Imprimir todas
    </button>
</div>
...
@php
    ...
    $facturasCliente = $item['facturasCliente'];
    ...
    <div class="boleta">
        ...
        <div class="footer-row">
            ...
            <div class="firma-area">
                ...
                <div class="copias-info">Original: Cliente&nbsp;&nbsp;&nbsp;Duplicado: Empresa</div>
            </div>
        </div>
